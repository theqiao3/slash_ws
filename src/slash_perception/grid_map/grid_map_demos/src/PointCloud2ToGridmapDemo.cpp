...
bool PointCloud2ToGridmapDemo::readParameters()
{
    ...
    this->declare_parameter("point_min_z", rclcpp::ParameterValue(-1.0));
    this->declare_parameter("point_max_z", rclcpp::ParameterValue(2.0));
    ...
    this->get_parameter("point_min_z", point_min_z_);
    this->get_parameter("point_max_z", point_max_z_);
...
void PointCloud2ToGridmapDemo::pointCloud2Callback(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
{
    ...
    for (size_t i = 0; i < cloud_trans->points.size(); i++)
    {
        ...
        if (dxyz.norm() > point_min_dist_ && dxyz.norm() < point_max_dist_ && dxyz.z() > point_min_z_ && dxyz.z() < point_max_z_)
